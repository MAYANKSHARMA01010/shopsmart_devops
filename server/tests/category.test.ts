...
describe('ShopSmart — Category API Tests', () => {
  ...
  it('GET /api/categories returns a category tree', async () => {
  ...
  it('GET /api/categories/:id returns a single category', async () => {
    const res = await request(app).get(`/api/categories/${parentCategoryId}`);
    expect(res.status).toBe(200);
  ...
  it('POST /api/categories rejects non-admin users', async () => {
    const res = await request(app)
      .post('/api/categories')
  ...
  it('PUT /api/categories/:id updates a category (admin only)', async () => {
    const res = await request(app)
  ...
  it('DELETE /api/categories/:id blocks deletion when products exist', async () => {
    const res = await request(app)
      .delete(`/api/categories/${categoryWithProductId}`)
      .set('Authorization', `Bearer ${adminToken}`);

    expect(res.status).toBe(409);
  });

  it('DELETE /api/categories/:id deletes a category with no products', async () => {
    const res = await request(app)
      .delete(`/api/categories/${categoryForDeleteId}`)
      .set('Authorization', `Bearer ${adminToken}`);
